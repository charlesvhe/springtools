config_item用来保存一行配置，其中字段varchar1-5，text1-3，decimal1-3，timestamp1-3在不同的meta_code有不同的含义
config_meta保存了同一个meta_code中的字段业务属性与其数据库列名的对应关系
...
下列数据实例定义了一个userType的配置，varchar1存储code、varchar2存储name、varchar3存储description，varchar4存储img属性；
config_item插入了几条数据，第1条数据表示：1条配置数据，对应code=user、name=普通用户、description=刚注册还未认证用户、img=1.jpg信息
